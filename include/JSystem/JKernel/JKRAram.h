...
class JKRAramPiece {
  public:
#ifdef TARGET_PC
    static JKRAMCommand* prepareCommand(int direction, void* mramAddress, aram_addr_t aramAddress, u32 length,
                                        JKRAramBlock* aramBlock, JKRAMCommand::AMCommandCallback callback);
    static JKRAMCommand* orderAsync(int direction, void* mramAddress, aram_addr_t aramAddress, u32 length,
                                    JKRAramBlock* aramBlock, JKRAMCommand::AMCommandCallback callback);
    static bool orderSync(int direction, void* mramAddress, aram_addr_t aramAddress, u32 length,
...
#ifdef TARGET_PC
inline bool JKRAramPcs(int direction, void* mramAddress, aram_addr_t aramAddress, u32 length, JKRAramBlock* block) {
    return JKRAramPiece::orderSync(direction, mramAddress, aramAddress, length, block);
}
#else
...
inline bool JKRAramPcsHost(int direction, void* mramAddress, u32 aramAddress, u32 length, JKRAramBlock* block) {
#ifdef TARGET_PC
    return JKRAramPiece::orderSync(direction, mramAddress, aramAddress, length, block);
#else
    if (direction == ARAM_DIR_ARAM_TO_MRAM) {
        return JKRAramPiece::orderSync(direction, aramAddress, (u32)mramAddress, length, block);
    }
    return JKRAramPiece::orderSync(direction, (u32)mramAddress, aramAddress, length, block);
#endif
}
