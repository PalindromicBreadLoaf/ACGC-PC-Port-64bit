...
def generate_pc_assets_c(central_entries, init_func_names, dry_run):
    ...
    lines.append('static u8* load_file(const char* path, size_t* out_size) {')
    lines.append('    FILE* f = fopen(path, "rb");')
    lines.append('    long file_size;')
    lines.append('    size_t sz;')
    lines.append('    u8* buf;')
    lines.append('    if (!f) return NULL;')
    lines.append('    if (fseek(f, 0, SEEK_END) != 0 || (file_size = ftell(f)) < 0 || fseek(f, 0, SEEK_SET) != 0) {')
    lines.append('        fclose(f);')
    lines.append('        return NULL;')
    lines.append('    }')
    lines.append('    sz = (size_t)file_size;')
    lines.append('    buf = (u8*)malloc(sz);')
    lines.append('    if (buf && fread(buf, 1, sz, f) != sz) {')
    lines.append('        free(buf);')
    lines.append('        buf = NULL;')
    lines.append('    }')
    lines.append('    fclose(f);')
    lines.append('    if (out_size) *out_size = buf ? sz : 0;')
    ...
    lines.append('')
    lines.append('    /* Try disc image (CISO/ISO/GCM) — pc_disc_init() already called from main */')
    lines.append('    if (pc_disc_is_open()) {')
    lines.append('        g_dol_data = pc_disc_extract_dol(&g_dol_size);')
    lines.append('        g_rel_data = pc_disc_extract_rel(&g_rel_size);')
    lines.append('        if (g_dol_data && g_rel_data) {')
    lines.append('            pc_dol_view dol;')
    lines.append('            pc_rel_view rel;')
    lines.append('            rom_mode = pc_dol_open(&dol, g_dol_data, g_dol_size) && pc_rel_open(&rel, g_rel_data, g_rel_size);')
    lines.append('        }')
    lines.append('    }')
    lines.append('')
    ...
    lines.append('        g_rel_data = load_file("orig/GAFE01_00/files/foresta.rel.szs", &g_rel_size);')
    lines.append('        g_dol_data = load_file("orig/GAFE01_00/sys/main.dol", &g_dol_size);')
    lines.append('        if (g_rel_data && g_dol_data) {')
    lines.append('            pc_dol_view dol;')
    lines.append('            pc_rel_view rel;')
    lines.append('            rom_mode = pc_dol_open(&dol, g_dol_data, g_dol_size) && pc_rel_open(&rel, g_rel_data, g_rel_size);')
    lines.append('            if (rom_mode && g_pc_verbose) printf("[PC] ROM-direct mode: loaded pre-extracted DOL + REL\\n");')
    lines.append('        } else {')
    lines.append('            if (g_rel_data) { free(g_rel_data); g_rel_data = NULL; }')
    lines.append('            if (g_dol_data) { free(g_dol_data); g_dol_data = NULL; }')
    lines.append('        }')
    ...
    lines.append('    if (!rom_mode) {')
    lines.append('        if (g_rel_data) { free(g_rel_data); g_rel_data = NULL; g_rel_size = 0; }')
    lines.append('        if (g_dol_data) { free(g_dol_data); g_dol_data = NULL; g_dol_size = 0; }')
