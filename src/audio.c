...
extern void sAdo_RhythmStart(runtime_id_t p, s8 type, s8 wait) {
    Na_RhythmStart(p, type, wait);
}

extern void sAdo_RhythmStop(runtime_id_t p) {
    Na_RhythmStop(p);
}
...
extern f32 sAdo_GetRhythmAnimCounter(runtime_id_t p) {
    return Na_GetRhythmAnimCounter(p);
}

extern s8 sAdo_GetRhythmDelay(runtime_id_t p) {
...
extern void sAdo_RhythmPos(runtime_id_t p, u8 haniwa_id, const xyz_t* pos) {
    u16 scalc;
    f32 fcalc;
...
extern void sAdos_KishaStatusLevel(f32 speed, runtime_id_t ongenNum1, f32 distance1, u16 angle1,
                                   runtime_id_t ongenNum2, f32 distance2, u16 angle2) {
