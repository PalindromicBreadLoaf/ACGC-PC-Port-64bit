 ...
 * Size:	000070
 */
static DSPTask* WriteTask(u8 target, dsp_cmd_addr_t cmd, void* task, DSPCallback callback)
{
	if (TASK_REMAIN == 4) {
		return NULL;
	}
...
static void DoTask()
{
	...
	while (TASK_REMAIN != 0) {
		...
		dsp_cmd_addr_t cmd = dspTask->cmd;
...
{
	while (WriteTask(DSPTARGET_IPL, (dsp_cmd_addr_t)cmd, task, callback) == NULL) {
		;
	}
}
...
{
	while (WriteTask(DSPTARGET_AGB, (dsp_cmd_addr_t)cmd, task, callback) == NULL) {}
}
