...
JKRAramBlock* JKRAram::mainRamToAram(u8* buf, u32 address, u32 alignedSize, JKRExpandSwitch expandSwitch, u32 fileSize,
    ...
    if (expandSwitch == EXPAND_SWITCH_DECOMPRESS) {
        ...
        if (allocatedMem == nullptr) {
            ...
            JKRAramPcsHost(0, allocatedMem, address, alignedSize, block);
        ...
        JKRAramPcsHost(0, buf, address, alignedSize, block);
        block = block == nullptr ? (JKRAramBlock*)-1 : block;
    }
    return block;
}
...
u8* JKRAram::aramToMainRam(u32 address, u8* buf, u32 srcSize, JKRExpandSwitch expandSwitch, u32 p5, JKRHeap* heap,
...
#endif
        JKRAramPcsHost(1, bufPtr, address, sizeof(buffer) / 2,
    ...
    } else if (compression == JKRCOMPRESSION_YAY0) // SZP
    {
        ...
        if (szpSpace == nullptr) {
            ...
            JKRAramPcsHost(1, szpSpace, address, srcSize, nullptr);
            if (p5 != 0 && p5 < expandSize)
                expandSize = p5;
    ...
    } else // Not compressed or ASR
    {
        ...
        if (buf == nullptr) {
            ...
            changeGroupIdIfNeed(buf, id);
            JKRAramPcsHost(1, buf, address, srcSize, nullptr);
            if (pSize != nullptr) {
                *pSize = srcSize;
            }
            return buf;
...
static u8* firstSrcData() {
    ...
    JKRAramPcsHost(1, buf, srcAddress + srcOffset, ALIGN_NEXT(transSize, 32), nullptr);
...
u8* nextSrcData(u8* current) {
    ...
    JKRAramPcsHost(1, dest + left, srcAddress + srcOffset, ALIGN_NEXT(transSize, 0x20), nullptr);
    srcOffset += transSize;
    transLeft -= transSize;
