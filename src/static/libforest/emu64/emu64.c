...
#define INVALID_TEX_FORMAT ((u16)-1)
__declspec(section ".rodata") const u16 emu64::fmtxtbl[8][4] = {
    { GX_TF_CMPR, INVALID_TEX_FORMAT, GX_TF_RGB5A3, GX_TF_RGBA8 }, /* G_IM_FMT_RGBA */
    { INVALID_TEX_FORMAT, INVALID_TEX_FORMAT, INVALID_TEX_FORMAT, INVALID_TEX_FORMAT }, /* G_IM_FMT_YUV */
    { GX_TF_C4, GX_TF_C8, 0xA, INVALID_TEX_FORMAT },               /* G_IM_FMT_CI */
    { INVALID_TEX_FORMAT, GX_TF_IA4, GX_TF_IA8, INVALID_TEX_FORMAT }, /* G_IM_FMT_IA */
    { GX_TF_I4, GX_TF_I8, GX_TF_RGB565, INVALID_TEX_FORMAT },      /* G_IM_FMT_I */
    { GX_TF_CMPR, GX_CTF_A8, GX_TF_RGB5A3, INVALID_TEX_FORMAT },   /* ?? */
    { INVALID_TEX_FORMAT, GX_TF_Z8, GX_TF_Z16, GX_TF_Z24X8 },      /* ?? */
    { INVALID_TEX_FORMAT, INVALID_TEX_FORMAT, INVALID_TEX_FORMAT, INVALID_TEX_FORMAT } /* ?? */
};
#undef INVALID_TEX_FORMAT
...
/* @fabricated */
static size_t texture_cache_get_max_alloc_size(texture_cache_t* cache) {
    return cache->buffer_current - cache->buffer_start;
}

/* @fabricated */
static size_t texture_cache_get_alloc_size(texture_cache_t* cache) {
    return cache->buffer_current - cache->last_alloc_start;
}

/* @fabricated */
static size_t texture_cache_get_free_size(texture_cache_t* cache) {
    return cache->buffer_end - cache->buffer_current;
}

/* @fabricated */
static size_t texture_cache_get_heap_size(texture_cache_t* cache) {
    return cache->buffer_end - cache->buffer_start;
}

static void* texture_cache_alloc(texture_cache_t* cache, u32 size) {
    cache->last_alloc_start = cache->buffer_current;
    cache->last_alloc_end = (u8*)ALIGN_NEXT((uintptr_t)cache->buffer_current + size, (uintptr_t)32);
...
void emu64::setup_texture_tile(int tile) {
    ...
    if (setimg_new->setimg2.isDolphin == FALSE) {
        ...
        EMU64_WARNF(
            ...
            get_siz_str(settile->siz),
            twd0, tht0,
            (int)(texture_cache_data.buffer_current - texture_cache_data.buffer_start),
            (int)(texture_cache_bss.buffer_current - texture_cache_bss.buffer_start)
        );
    }
