...
static void aIBT_set_fly_se(aINS_INSECT_ACTOR* insect) {
    switch (insect->type) {
        case aINS_INSECT_TYPE_LONG_LOCUST:
            sAdo_OngenPos((runtime_id_t)insect, 0xA2, &insect->tools_actor.actor_class.world.position);
            break;
        case aINS_INSECT_TYPE_MIGRATORY_LOCUST:
            sAdo_OngenPos((runtime_id_t)insect, 0xA3, &insect->tools_actor.actor_class.world.position);
            break;
    }
}
...
static int aIBT_check_player_net_sub(xyz_t* net_pos, aINS_INSECT_ACTOR* insect) {
    ...
    if (mPlib_Check_StopNet(net_pos) == TRUE) {
        runtime_id_t label = mPlib_Get_item_net_catch_label();
        if (label == (runtime_id_t)insect) {
            ret = TRUE;
        }
    }
    return ret;
    ...
    if (actor->bg_collision_check.result.is_in_water) {
        ...
        if ((insect->type >= 0xF) || (insect->type < 0xD)) {
            ...
            if ((actor->bg_collision_check.result.on_ground) &&
                ...
                if (insect->s32_work1 == 0) {
                    sAdo_OngenPos((runtime_id_t)insect, batta_sound_data[idx], &actor->world.position);
                }
            }
        }
        if (insect->timer > 0) {
