...
#define aIGK_SET_ITEM_P(insect, item_p) ((insect)->s32_work3 = (runtime_id_t)item_p)
...
static void aIGK_move_on_flower(ACTOR* actorx, GAME* game) {
    ...
    if (aIGK_check_patience(insect) == TRUE) {
        ...
        if (aIGK_MOVE_TIMER(insect) <= 0.0f) {
            sAdo_OngenPos((runtime_id_t)actorx, NA_SE_GOKI_MOVE, &actorx->world.position);
