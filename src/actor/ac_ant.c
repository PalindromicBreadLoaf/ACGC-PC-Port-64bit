...
static void aANT_wait(ANT_ACTOR* ant, GAME* game) {
    if (ant->below_fg_p == NULL || (*ant->below_fg_p != ITM_FOOD_CANDY && *ant->below_fg_p != ITM_KABU_SPOILED)) {
        ...
        runtime_id_t catch_label = mPlib_Get_item_net_catch_label();

        if ((runtime_id_t)ant == catch_label) {
            ...
            if ((catch_swing_timer > 0.0f || mPlib_Check_StopNet(&stop_net_pos)) && ant->actor_class.player_distance_xz < mFI_UNIT_BASE_SIZE_F) {
                GET_PLAYER_ACTOR_NOW()->Set_Item_net_catch_request_force_proc(GET_PLAYER_ACTOR_NOW_ACTOR(), gamePT, (runtime_id_t)ant, mPlayer_NET_CATCH_TYPE_ANT);
            } else {
                GET_PLAYER_ACTOR_NOW()->Set_Item_net_catch_request_table_proc(GET_PLAYER_ACTOR_NOW_ACTOR(), gamePT, (runtime_id_t)ant, mPlayer_NET_CATCH_TYPE_ANT, &ant->actor_class.world.position, 24.0f);
            }
        }
    }
}
...
static void aANT_caught(ANT_ACTOR* ant, GAME* game) {
    ...
    if (ant->insect_actor != NULL) {
        if (mPlib_Change_item_net_catch_label((runtime_id_t)ant->insect_actor, mPlayer_NET_CATCH_TYPE_INSECT)) {
            aANT_setupAction(ant, aANT_ACT_DISAPPEAR);
        }
    } else {
