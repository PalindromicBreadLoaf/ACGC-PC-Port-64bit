...
static void aBEE_appear(ACTOR* actorx, GAME* game) {
    ...
    actorx->shape_info.rotation.z += (s16)(DEG2SHORT_ANGLE2(22.5f) * dt);
...
static void aBEE_fly(ACTOR* actorx, GAME* game) {
    ...
    if ((runtime_id_t)bee == mPlib_Get_item_net_catch_label()) {
        ...
        if (mPlib_check_player_actor_main_index_Pitfall(game)) {
            ...
            if (bee->catch_delay_frames > 0) {
                ...
                if ((swing_timer > 0.0f || mPlib_Check_StopNet(&pos)) && actorx->player_distance_xz < 40.0f) {
                    GET_PLAYER_ACTOR_NOW()->Set_Item_net_catch_request_force_proc(GET_PLAYER_ACTOR_NOW_ACTOR(), gamePT, (runtime_id_t)actorx, TRUE);
                } else {
                    GET_PLAYER_ACTOR_NOW()->Set_Item_net_catch_request_table_proc(GET_PLAYER_ACTOR_NOW_ACTOR(), gamePT, (runtime_id_t)actorx, TRUE, &actorx->world.position, 24.0f);
                }
            }
...
static void aBEE_caught(ACTOR* actorx, GAME* game) {
    ...
    if (bee->insect_actor != NULL) {
        if (mPlib_Change_item_net_catch_label((runtime_id_t)bee->insect_actor, 0)) {
            aBEE_setupAction(bee, aBEE_ACT_DISAPPEAR, game);
            return;
        }
    } else {
...
static void aBEE_actor_move(ACTOR* actorx, GAME* game) {
    ...
    sAdo_OngenPos((runtime_id_t)actorx, NA_SE_B0, &actorx->world.position);
...
static void aBEE_actor_draw(ACTOR* actorx, GAME* game) {
    ...
    if (gfx != NULL) {
        ...
        gDPSetPrimColor(POLY_XLU_DISP++, 0, 255, 0, 0, 0, (u8)bee->alpha);
