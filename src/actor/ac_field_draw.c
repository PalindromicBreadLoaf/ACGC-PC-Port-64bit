        ...
        if (source_num != 0) {
            ...
            for (i = 0; i < ARRAY_COUNT(ongen_info); i++) {
                ...
                if (info->sound_source_no != -1) {
                    runtime_id_t ongen_id = (runtime_id_t)actorx + (mFI_GetBlockXMax() * info->bz + info->bx) * mFM_SOUND_SOURCE_NUM +
    ...
    if (mFI_Wpos2BlockNum(&bx, &bz, player->actor_class.world.position)) {
        ...
        if (mRF_SearchPond(&pond_x, &pond_z, bx, bz)) {
            ...
            if (rtc_time->month >= lbRTC_MAY && rtc_time->month <= lbRTC_AUGUST &&
                ...
                sAdo_OngenPos((runtime_id_t)&Bg_Draw_Actor_move, 0xA1, &frog_se_pos);
            }
            ...
            sAdo_OngenPos((runtime_id_t)&Bg_Draw_Actor_ct, 0x16, &pond_pos);
        }
    }
